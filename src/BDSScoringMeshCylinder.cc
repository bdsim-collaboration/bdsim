  ...
  if (recipe.rInner > 0)
    {
      ...
      msg += "rInner: " + std::to_string(recipe.rInner) + " mm in mesh \"" + name +"\"";
      throw BDSException(__METHOD_NAME__, msg);
    }
  scorerSize[1] = recipe.ScoringMeshZ(); // full z-length
#else
  scorerSize[0] = recipe.rInner;
  scorerSize[1] = recipe.rOuter;
